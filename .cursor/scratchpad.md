...
- Per risolvere il problema di scroll verticale su mobile nella pagina `/clienti` quando la tabella ha molte righe, ho inizialmente rimosso `overflow-hidden` dal wrapper principale di `ClientsTable` (`<main className="...">`) per permettere alla card di espandersi. Dopo ulteriore feedback ho allineato il comportamento a quello delle pagine trattative: il wrapper principale ora include di nuovo `overflow-hidden` e lo scroll verticale avviene solo nel contenitore interno `scroll-fade-y` che avvolge la tabella, così che l'header e le stat cards restino fissi e l'utente scorra solo la lista dei clienti.
 - Per risolvere il problema di scroll verticale su mobile nella pagina `/clienti` quando la tabella ha molte righe, ho inizialmente rimosso `overflow-hidden` dal wrapper principale di `ClientsTable` (`<main className="...">`) per permettere alla card di espandersi. Dopo ulteriore feedback ho allineato il comportamento a quello delle pagine trattative: il wrapper principale ora include di nuovo `overflow-hidden` e lo scroll verticale avviene solo nel contenitore interno `scroll-fade-y` che avvolge la tabella, così che l'header e le stat cards restino fissi e l'utente scorra solo la lista dei clienti.
- In seguito al nuovo feedback su `/clienti` (impossibilità di scorrere la tabella su mobile), ho raffinato ulteriormente il contenitore di scroll in `ClientsTable`: ora l'overflow orizzontale (`overflow-x-auto`) è applicato al wrapper che contiene header + righe, mentre lo scroll verticale (`overflow-y-auto`) è limitato al solo blocco `scroll-fade-y` delle righe. In questo modo su mobile è possibile scorrere verso il basso per vedere più righe senza muovere l'intera card/pagina, mantenendo al tempo stesso lo scroll orizzontale condiviso tra header e body.
