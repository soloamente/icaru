...
- [ ] Abilitare lo scroll verticale su mobile per la tabella clienti quando l'elenco supera l'altezza dello schermo (`/clienti`).
 - [ ] Abilitare lo scroll verticale su mobile per la tabella trattative quando l'elenco supera l'altezza dello schermo (`/trattative/*`).
...
- Per risolvere il problema di scroll verticale su mobile nella pagina `/clienti` quando la tabella ha molte righe, ho rimosso la classe `overflow-hidden` dal wrapper principale di `ClientsTable` (`<main className="...">`). In questo modo, quando il contenuto complessivo della card supera l'altezza disponibile, l'intera card può estendersi e la pagina può scrollare normalmente anche su viewport stretti, mantenendo invariata la struttura interna con il container `scroll-fade-y` per lo scroll della tabella. Chiedo una verifica manuale su mobile (viewport ~430×932) per confermare che sia ora possibile scorrere la lista dei clienti fino in fondo.

- Ho applicato la stessa correzione di scroll anche alle pagine trattative (`TrattativeTable` usata su `/trattative/tutte`, `/trattative/aperte`, `/trattative/concluse`, `/trattative/abbandonate`): ho rimosso `overflow-hidden` dal wrapper principale `<main>` in `trattative-table.tsx`. Ora, quando la tabella delle trattative supera in altezza lo schermo su mobile, la card può espandersi e l'intera pagina può scorrere verticalmente, mentre il container interno `scroll-fade-y` continua a gestire lo scroll del corpo tabellare in modo coerente con desktop.
